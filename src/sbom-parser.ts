...
export class SBOMParser {
        ...
        try {
            ...
            switch (format) {
                case SBOMFormat.CYCLONEDX:
                    return this.parseCycloneDX(data as CycloneDXBOM);
                case SBOMFormat.SPDX:
                    return this.parseSPDX(data as SPDXDocument);
                default:
                    throw new Error(`Unsupported SBOM format: ${format}`);
            }
        } catch (error) {
            throw new Error(
                `Failed to parse SBOM file: ${getErrorMessage(error)}`
            );
        }
    }
     ...
     * Parse CycloneDX BOM
     */
    private static parseCycloneDX(bom: CycloneDXBOM): Map<string, string> {
        ...
        const extractComponents = (comps: CycloneDXComponent[]) => {
            for (const component of comps) {
                if (component.name && component.version) {
                    ...
                    const productName = this.mapComponentToProduct(component.name);
                    if (productName) {
     ...
     * Parse SPDX document
     */
    private static parseSPDX(doc: SPDXDocument): Map<string, string> {
        const components = new Map<string, string>();
        ...
        for (const pkg of doc.packages) {
            if (pkg.name && pkg.versionInfo) {
                const productName = this.mapComponentToProduct(pkg.name);
                if (productName) {
     ...
     */
    private static mapComponentToProduct(componentName: string): string | null {
